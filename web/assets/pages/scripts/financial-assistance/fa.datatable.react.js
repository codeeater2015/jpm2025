var FinancialAssistanceDatatable = React.createClass({
    ...
    initDatatable: function () {
        ...
        var financial_assistance_table = $("#financial_assistance_table");
        var grid_project_event = new Datatable();
        var url = Routing.generate("ajax_get_datatable_financial_assistance", {}, true);

        grid_project_event.init({
            src: financial_assistance_table,
            ...
            "dataTable": {
                ...
                "ajax": {
                    ...
                    "type": 'GET',
                    "data": function (d) {
                        d.trnNo = $('#financial_assistance_table input[name="trn_no"]').val();
                        d.trnDate = $('#financial_assistance_table input[name="trn_date]').val();
                        d.applicantName = $('#financial_assistance_table input[name="applicant_name"]').val();
                        d.beneficiaryName = $('#financial_assistance_table input[name="beneficiary_name"]').val();
                        d.endorsedBy = $('#financial_assistance_table input[name="endorsed_by"]').val();
                        d.municipalityName = $('#financial_assistance_table input[name="municipality_name"]').val();
                        d.barangayName = $('#financial_assistance_table input[name="barangay_name"]').val();
                    }
                },
                "columnDefs": [{
                    'orderable': false,
        ...
        financial_assistance_table.on('click', '.edit-button', function () {
            var data = grid_project_event.getDataTable().row($(this).parents('tr')).data();
            self.setState({ showEditModal: true, target: data.trn_id });
        });

        financial_assistance_table.on('click', '.release-button', function () {
            var data = grid_project_event.getDataTable().row($(this).parents('tr')).data();
            self.setState({ showReleaseModal: true, target: data.trn_id });
        });

        financial_assistance_table.on('click', '.delete-button', function () {
            var data = grid_project_event.getDataTable().row($(this).parents('tr')).data();
            self.delete(data.trn_id);
        });
    ...
    delete: function (trnId) {
        var self = this;

        if (confirm("Are you sure you want to delete this request?")) {
            self.requestDelete = $.ajax({
                url: Routing.generate("ajax_delete_financial_assistance", { trnId: trnId }),
                type: "DELETE"
            }).done(function (res) {
                self.reload();
            });
    ...
    render: function () {
        ...
        return (
            <div>
                ...
                <div className="row" id="fa_component">
                    ...
                    <div className="col-md-2">
                        <select className="form-control" onChange={this.setActiveTable} name="activeTable">
                ...
                <div className="table-container" style={{ marginTop: "20px" }}>
                    <table id="financial_assistance_table" className="table table-striped table-bordered" width="100%">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Trn No</th>
...
window.FinancialAssistanceDatatable = FinancialAssistanceDatatable;