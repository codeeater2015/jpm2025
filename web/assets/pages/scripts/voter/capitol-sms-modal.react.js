...
var CapitolSmsModal = React.createClass({
        ...
        return (
            <Modal keyboard={false} enforceFocus={false} dialogClassName="modal-full" show={this.props.show} onHide={this.props.onHide}>
                <Modal.Header className="modal-header bg-blue-dark font-white bold" closeButton>
                    <Modal.Title>Capitol SMS Form</Modal.Title>
                </Modal.Header>
                <Modal.Body bsClass="modal-body overflow-auto">
                    ...
                    <form id="sms-form" onSubmit={this.submit}>
                        <div className="col-md-3">
                            ...
                            <FormGroup controlId="formMunicipality" validationState={this.getValidationState('municipality')}>
                                <ControlLabel > Municipality : </ControlLabel>
                                <select id="form-municipality-select2" className="form-control input-sm">
                                </select>
                                <HelpBlock>{this.getError('municipality')}</HelpBlock>
                            </FormGroup>

                            <FormGroup controlId="formBarangay" validationState={this.getValidationState('barangay')}>
                                <ControlLabel > Barangay : </ControlLabel>
                                <select id="form-barangay-select2" className="form-control input-sm">
                                </select>
                                <HelpBlock>{this.getError('barangay')}</HelpBlock>
                            </FormGroup>
    ...
    initSelect2: function () {
        ...
        $("#form-municipality-select2").select2({
            ...
            ajax: {
                url: Routing.generate('ajax_select2_capitol_municipality'),
                ...
                processResults: function (data, params) {
                    return {
                        results: data.map(function (item) {
                            return { id: item.municipality, text: item.municipality };
                        })
                    };
                },
            }
        ...
        $("#form-barangay-select2").select2({
            ...
            containerCssClass: ':all:',
            ajax: {
                url: Routing.generate('ajax_select2_capitol_barangay'),
                data: function (params) {
                    return {
                        searchText: params.term
                    };
                ...
                processResults: function (data, params) {
                    return {
                        results: data.map(function (item) {
                            return { id: item.barangay, text: item.barangay };
                        })
                    };
                },
            }
        ...
        $("#form-municipality-select2").on("change", function () {
            self.setFormPropValue("municipality", $(this).val());
        });

        $("#form-barangay-select2").on("change", function () {
            self.setFormPropValue("barangay", $(this).val());
        });
    },
    ...
    submit: function (e) {
        ...
        self.requestValidation = $.ajax({
            url: Routing.generate("ajax_post_capitol_sms"),
            type: "POST",
            data: data,
            xhrFields: {
...
window.CapitolSmsModal = CapitolSmsModal;