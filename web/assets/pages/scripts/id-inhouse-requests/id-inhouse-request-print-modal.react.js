...
var IdInhouseRequestPrintModal = React.createClass({
    ...
    loadHeader : function(hdrId){
        ...
        self.requestHeader = $.ajax({
            url : Routing.generate("ajax_get_id_request_header",{ hdrId : hdrId }),
            type : "GET"
        }).done(function(res){
            console.log(res);
            self.setState({ header : res });
        }).fail(function(err){
            console.log("failed to get the header");
        });
    },
	...
	submit : function(e){
        ...
        data.profiles = data.projectVoters;
        data.printOrigin = 'INHOUSE';
        data.printDesc = self.state.header.submittedBy + " - " +  moment(new Date(self.state.header.submittedAt)).format('MMM DD, YYYY');
        data.originId  = self.props.hdrId;
        ...
        self.requestPostUpdate = $.ajax({
            ...
            self.props.onHide();
            self.showKfcPrintout(res.printId);
    ...
    showKfcPrintout: function (printId) {
        var url = "http://" + window.hostIp + ":8100/voter-report/web/voter/id-template/index.php?print_id=" + printId;
		...
		return (
			<Modal enforceFocus={false} backdrop="static" dialogClassName="modal-custom-85" show={this.props.show} onHide={this.props.onHide}>
				<Modal.Header closeButton>
					<Modal.Title>ID Printing Form</Modal.Title>
				</Modal.Header>
						...
						<FormGroup controlId="formProjectVoters" validationState={this.getValidationState('projectVoters')} >
							<select multiple ref="selectBox" className="searchable" id="voters_multiselect" name="projectVoters[]">
								{this.state.options.map(function (item) {
									return (<option key={item.dtl_id} value={item.pro_voter_id}>{item.voter_name} / {item.barangay_name} ({moment(item.updated_at).format('MMM DD, YYYY hh:mm A')})</option>)
								})}
							</select>
							<div className="text-right">
...
window.IdInhouseRequestPrintModal = IdInhouseRequestPrintModal;